...
def process_jni_remap(file_content):
    """
    JNI9~16를 JNI1~8로, IDX9~16를 IDX1~8로 재매핑하는 함수.
    """
...
def cleanup_temp_folder(n_clicks):
    ...
    temp_dir_list = [temp_dir, upload_dir]

    try:
        if (os.path.exists(temp_dir)) or (os.path.exists(upload_dir)):
            for temp_dr in temp_dir_list:
                shutil.rmtree(temp_dr, ignore_errors=True)
            return html.Div("임시 폴더(temp,uploads)가 삭제되었습니다.", className='status-success', style={'marginTop': '10px'})
        else:
            return html.Div("삭제할 임시 폴더가 없습니다.", style={'color': '#6e6e73', 'marginTop': '10px'})
    except Exception as e:
        return html.Div(f"삭제 실패: {e}", className='status-error', style={'marginTop': '10px'})